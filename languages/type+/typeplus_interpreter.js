...
function RunTypeCode(c) {
  ...
  for(i = 0; i < codeType.length; i++) {
    ...
    } else if (TypeLang.command === 'new function') {
      TypeLang.newFunc = {}; i++; TypeLang.newFunc.name = TypeLang.command; TypeLang.newsFunc.code = ""; TypeLang.main = {}; i++; TypeLang.command = codeType[i]; TypeLang.main.code = TypeSysFunc['let a main()'](TypeLang.command)
      for(TypeLang.main.i = 0; TypeLang.main.i < TypeLang.main.code.length; TypeLang.main.i++) {
        TypeLang.main.command = TypeLang.main.code[TypeLang.main.i]
        TypeLang.newsFunc.code = `${TypeLang.newsFunc.code}; ${TypeLang.main.command}`
      }
      TypeCommands[TypeLang.newFunc.name] = TypeLang.newFunc.code
    }
  }
}
